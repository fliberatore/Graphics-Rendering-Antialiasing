...
typedef struct{
	...
	GzTextureIndex startUV, endUV, currentUV;

	float slopeX, slopeZ;
	GzColor slopeColor;
	GzTextureIndex slopeUV;
} GzDDA_GS;
#endif
...
int GzRasterizerPhongShading(GzRender *render, GzCoord vertexList[NUM_VERTICES], GzCoord normalList[NUM_VERTICES], GzTextureIndex uvList[NUM_VERTICES]);
int GzRasterizerGouraudShading(GzRender *render, GzCoord vertexList[NUM_VERTICES], GzCoord normalList[NUM_VERTICES], GzTextureIndex uvList[NUM_VERTICES]);
...
bool GzPixelDrawn(GzCoord *point, GzDisplay *display);
...
int GzNewRender(GzRender **render, GzDisplay	*display)
{
	...
	pRender->offset[X] = pRender->offset[Y] = 0.0;
	...
	GzPointer	*valueList) /* void** valuelist */
{
	...
	for(unsigned int i = 0; i < numAttributes; ++i){
		...
		case GZ_TEXTURE_MAP:
			...
			break;
		case GZ_AASHIFTX:
			render->offset[X] = *((float*)(valueList[i]));
			break;
		case GZ_AASHIFTY:
			render->offset[Y] = *((float*)(valueList[i]));
			break;
...
*/
	...
	GzTextureIndex uvList[NUM_VERTICES];
	GzCoord triNorm;
	...
	for(unsigned int p = 0; p < numParts; ++p){
		...
		case GZ_POSITION:
			...
			//Applying offset
			for(unsigned int v = 0; v < NUM_VERTICES; ++v){
				vertexList[v][X] += render->offset[X];
				vertexList[v][Y] += render->offset[Y];
			}
		...
		case GZ_TEXTURE_INDEX:
			memcpy(uvList, valueList[p], NUM_VERTICES*sizeof(GzTextureIndex));
			hasUV = true;
			break;
	...
	case GZ_COLOR:
		//Gouraud Shading
		if(render->tex_fun == NULL && hasPosition && hasNormal) return GzRasterizerGouraudShading(render, vertexList, normalList, uvList);
		else if(render->tex_fun != NULL && hasPosition && hasNormal && hasUV) return GzRasterizerGouraudShading(render, vertexList, normalList, uvList);
		else return GZ_FAILURE;
		break;
	case GZ_NORMALS:
		//Phong Shading
		if(render->tex_fun == NULL && hasPosition && hasNormal) return GzRasterizerPhongShading(render, vertexList, normalList, uvList);
		else if(render->tex_fun != NULL && hasPosition && hasNormal && hasUV) return GzRasterizerPhongShading(render, vertexList, normalList, uvList);
		else return GZ_FAILURE;
		break;
	default:
		return GZ_FAILURE;
...
/* Gouraud-Shading Scan-Line Rasterizer algorithm with textures */
int GzRasterizerGouraudShading(GzRender *render, GzCoord vertexList[NUM_VERTICES], GzCoord normalList[NUM_VERTICES], GzTextureIndex uvList[NUM_VERTICES]){
	...
	if(render->tex_fun !=(GzTexture) NULL){
		GzColor white = {1,1,1};
		memcpy(render->Ka, white, sizeof(GzColor));
		memcpy(render->Kd, white, sizeof(GzColor));
		memcpy(render->Ks, white, sizeof(GzColor));
	}
	...
	for(unsigned int i = 0; i < NUM_VERTICES; ++i){
		float Zp = vertexList[i][Z] / (MAXINT - vertexList[i][Z]);
		uvList[i][U] /= Zp + 1;
		uvList[i][V] /= Zp + 1;
	}
	...
	for(unsigned int i = 0; i < NUM_VERTICES-1; ++i){
		for(unsigned int j = i+1; j < NUM_VERTICES; ++j){
			if(vertexList[i][Y] > vertexList[j][Y]){
				...
				memcpy(tmpUV, uvList[i], sizeof(GzTextureIndex));
				memcpy(uvList[i], uvList[j], sizeof(GzTextureIndex));
				memcpy(uvList[j], tmpUV, sizeof(GzTextureIndex));
			}
		}
	}
	float Ymax = vertexList[NUM_VERTICES-1][Y];
	...
	unsigned int DDA_NC_size = 0;
	{
		...
		memcpy(DDAList[0].startUV, uvList[0], sizeof(GzTextureIndex));
		memcpy(DDAList[0].currentUV, uvList[0], sizeof(GzTextureIndex));
		memcpy(DDAList[0].endUV, uvList[1], sizeof(GzTextureIndex));
		if(DDAList[0].end[Y] != DDAList[0].start[Y]){
			DDAList[0].slopeX = (DDAList[0].end[X] - DDAList[0].start[X])/(DDAList[0].end[Y] - DDAList[0].start[Y]);
			DDAList[0].slopeZ = (DDAList[0].end[Z] - DDAList[0].start[Z])/(DDAList[0].end[Y] - DDAList[0].start[Y]);
			...
			DDAList[0].slopeUV[U] = (DDAList[0].endUV[U] - DDAList[0].startUV[U])/(DDAList[0].end[Y] - DDAList[0].start[Y]);
			DDAList[0].slopeUV[V] = (DDAList[0].endUV[V] - DDAList[0].startUV[V])/(DDAList[0].end[Y] - DDAList[0].start[Y]);
			DDA_NC[DDA_NC_size++] = &(DDAList[0]);
		}else{
			DDAList[0].slopeX = DDAList[0].slopeZ = DDAList[0].slopeColor[RED] = DDAList[0].slopeColor[GREEN] = DDAList[0].slopeColor[BLUE] = 0;
			DDAList[0].slopeUV[U] = DDAList[0].slopeUV[V] = 0;
			DDA_H = &DDAList[0];
			++num_h;
			DDA_H_isTop = true;
		}
		...
		memcpy(DDAList[1].startUV, uvList[0], sizeof(GzTextureIndex));
		memcpy(DDAList[1].currentUV, uvList[0], sizeof(GzTextureIndex));
		memcpy(DDAList[1].endUV, uvList[2], sizeof(GzTextureIndex));
		if(DDAList[1].end[Y] != DDAList[1].start[Y]){
			DDAList[1].slopeX = (DDAList[1].end[X] - DDAList[1].start[X])/(DDAList[1].end[Y] - DDAList[1].start[Y]);
			DDAList[1].slopeZ = (DDAList[1].end[Z] - DDAList[1].start[Z])/(DDAList[1].end[Y] - DDAList[1].start[Y]);
			...
			DDAList[1].slopeUV[U] = (DDAList[1].endUV[U] - DDAList[1].startUV[U])/(DDAList[1].end[Y] - DDAList[1].start[Y]);
			DDAList[1].slopeUV[V] = (DDAList[1].endUV[V] - DDAList[1].startUV[V])/(DDAList[1].end[Y] - DDAList[1].start[Y]);
			DDA_NC[DDA_NC_size++] = &(DDAList[1]);
		}else{
			DDAList[1].slopeX = DDAList[1].slopeZ = DDAList[1].slopeColor[RED] = DDAList[1].slopeColor[GREEN] = DDAList[1].slopeColor[BLUE] = 0;
			DDAList[1].slopeUV[U] = DDAList[1].slopeUV[V] = 0;
			DDA_H = &DDAList[1];
			++num_h;
		}
		if(DDAList[1].end[X] == DDAList[1].start[X]) ++num_v;
		...
		memcpy(DDAList[2].startUV, uvList[1], sizeof(GzTextureIndex));
		memcpy(DDAList[2].currentUV, uvList[1], sizeof(GzTextureIndex));
		memcpy(DDAList[2].endUV, uvList[2], sizeof(GzTextureIndex));
		if(DDAList[2].end[Y] != DDAList[2].start[Y]){
			DDAList[2].slopeX = (DDAList[2].end[X] - DDAList[2].start[X])/(DDAList[2].end[Y] - DDAList[2].start[Y]);
			DDAList[2].slopeZ = (DDAList[2].end[Z] - DDAList[2].start[Z])/(DDAList[2].end[Y] - DDAList[2].start[Y]);
			...
			DDAList[2].slopeUV[U] = (DDAList[2].endUV[U] - DDAList[2].startUV[U])/(DDAList[2].end[Y] - DDAList[2].start[Y]);
			DDAList[2].slopeUV[V] = (DDAList[2].endUV[V] - DDAList[2].startUV[V])/(DDAList[2].end[Y] - DDAList[2].start[Y]);
			DDA_NC[DDA_NC_size++] = &(DDAList[2]);
		}else{
			DDAList[2].slopeX = DDAList[2].slopeZ  = DDAList[2].slopeColor[RED] = DDAList[2].slopeColor[GREEN] = DDAList[2].slopeColor[BLUE] = 0;
			DDAList[2].slopeUV[U] = DDAList[2].slopeUV[V] = 0;
			DDA_H = &DDAList[2];
			++num_h;
			DDA_H_isTop = false;
		}
	...
	while(Ypos < Ymax){
		...
		if(DDA_L[pos_L]->current[Y] != Ypos){
			...
			DDA_L[pos_L]->currentUV[U] += deltaY * DDA_L[pos_L]->slopeUV[U];
			DDA_L[pos_L]->currentUV[V] += deltaY * DDA_L[pos_L]->slopeUV[V];
		}
		if(DDA_R[pos_R]->current[Y] != Ypos){
			deltaY = Ypos - DDA_R[pos_R]->current[Y];
			DDA_R[pos_R]->current[X] += deltaY * DDA_R[pos_R]->slopeX;
			...
			DDA_R[pos_R]->currentUV[U] += deltaY * DDA_R[pos_R]->slopeUV[U];
			DDA_R[pos_R]->currentUV[V] += deltaY * DDA_R[pos_R]->slopeUV[V];
		}
		...
		memcpy(span.startUV, DDA_L[pos_L]->currentUV, sizeof(GzTextureIndex));
		memcpy(span.endUV, DDA_R[pos_R]->currentUV, sizeof(GzTextureIndex));
		memcpy(span.currentUV, DDA_L[pos_L]->currentUV, sizeof(GzTextureIndex));
		span.slopeZ = (DDA_R[pos_R]->current[Z] - DDA_L[pos_L]->current[Z])/(DDA_R[pos_R]->current[X] - DDA_L[pos_L]->current[X]);
		...
		span.slopeUV[U] = (DDA_R[pos_R]->currentUV[U] - DDA_L[pos_L]->currentUV[U])/(DDA_R[pos_R]->current[X] - DDA_L[pos_L]->current[X]);
		span.slopeUV[V] = (DDA_R[pos_R]->currentUV[V] - DDA_L[pos_L]->currentUV[V])/(DDA_R[pos_R]->current[X] - DDA_L[pos_L]->current[X]);
		...
		span.currentUV[U] += deltaX * span.slopeUV[U];
		span.currentUV[V] += deltaX * span.slopeUV[V];
		...
		if(!(num_h == 1 && !DDA_H_isTop && Ypos == Ymax)){
			...
			//NOT DRAWING pixels covered by right edges
			while(span.current[X] < span.end[X] && span.current[X] <= render->display->xres){
				if( GzPixelDrawn(&(span.current), render->display) ){
					//Getting texture color (if texture)
					if(render->tex_fun != NULL){
						GzColor currentColor;
						//Texture mapping
						float Zp = span.current[Z] / (MAXINT - span.current[Z]);
						render->tex_fun(span.currentUV[U] * (Zp + 1), span.currentUV[V] * (Zp + 1), currentColor);
						currentColor[RED] *= span.currentColor[RED];
						currentColor[GREEN] *= span.currentColor[GREEN];
						currentColor[BLUE] *= span.currentColor[BLUE];
						
						if(GzPixelToDisplay(&(span.current), render->display, &(currentColor)))
						return GZ_FAILURE;
					}
					else if(GzPixelToDisplay(&(span.current), render->display, &(span.currentColor)))
							return GZ_FAILURE;
				}
				span.current[X] += 1.0f;
				span.current[Z] += span.slopeZ;
				...
				span.currentUV[U] += span.slopeUV[U];
				span.currentUV[V] += span.slopeUV[V];
			}
		}
		//Advance to the next scan line
		Ypos += 1.0f;
...
/* Phong-Shading Scan-Line Rasterizer algorithm with textures */
int GzRasterizerPhongShading(GzRender *render, GzCoord vertexList[NUM_VERTICES], GzCoord normalList[NUM_VERTICES], GzTextureIndex uvList[NUM_VERTICES]){
	if(!render)
		return GZ_FAILURE;
	...
	unsigned int DDA_NC_size = 0;
	{
		...
		memcpy(DDAList[0].current, vertexList[0], sizeof(GzCoord));
		memcpy(DDAList[0].end, vertexList[1], sizeof(GzCoord));
		memcpy(DDAList[0].startNormal, normalList[0], sizeof(GzCoord));
		memcpy(DDAList[0].currentNormal, normalList[0], sizeof(GzCoord));
		memcpy(DDAList[0].endNormal, normalList[1], sizeof(GzCoord));
		memcpy(DDAList[0].startUV, uvList[0], sizeof(GzTextureIndex));
		memcpy(DDAList[0].currentUV, uvList[0], sizeof(GzTextureIndex));
		memcpy(DDAList[0].endUV, uvList[1], sizeof(GzTextureIndex));
		if(DDAList[0].end[Y] != DDAList[0].start[Y]){
		...
		if(!(num_h == 1 && !DDA_H_isTop && Ypos == Ymax)){
			...
			while(span.current[X] < span.end[X] && span.current[X] <= render->display->xres){
				...
				if(render->tex_fun != NULL){
					...
					float Zp = span.current[Z] / (MAXINT - span.current[Z]);
					render->tex_fun(span.currentUV[U] * (Zp + 1), span.currentUV[V] * (Zp + 1), currentColor);
					memcpy(render->Ka, currentColor, sizeof(GzColor));
					memcpy(render->Kd, currentColor, sizeof(GzColor));
				}
				if(GzPixelDrawn(&(span.current), render->display)){
